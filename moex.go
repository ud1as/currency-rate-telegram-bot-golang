...
func getRate(code string, fetch fetchFunction) (float32, float32, error) {
	...
	if c == (&Currency{}) {
		return float32(res), 0, fmt.Errorf("MOEX error: c is zero")
	}
	if len(c.values) < 2 {
		return float32(res), 0, fmt.Errorf("MOEX error: length of c.values less than 2")
	}
	val := c.values[1]
	if val.Marketdata == nil {
		return float32(res), 0, fmt.Errorf("MOEX error: val.Marketdata is zero")
	}
	md := val.Marketdata
	if len(md) == 0 {
		return 0, 0, fmt.Errorf("MOEX error: length of md equals 0")
	}

	res = md[0].Last
	ch := res - md[0].Open

	return float32(res), float32(ch), nil
}
